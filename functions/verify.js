	...
	return got.get( wiki + 'api.php?action=query&meta=siteinfo' + ( wiki.wikifarm === 'fandom' ? '|allmessages&siprop=general&ammessages=importJS&list=users|usercontribs&ucprop=&uclimit=10&ucuser=%1F' + encodeURIComponent( username.replaceAll( '\x1F', '\ufffd' ) ) : '&siprop=general&list=users' ) + '&usprop=blockinfo|groups|editcount|registration|gender&ususers=%1F' + encodeURIComponent( username.replaceAll( '\x1F', '\ufffd' ) ) + '&format=json&requestid=cachebreak-' + Date.now(), {
		...
		if ( wiki.hasOAuth2() && process.env.dashboard ) {
			...
			if ( wiki.wikifarm === 'wikimedia' ) oauth.push('wikimedia');
				...
				else if ( Wiki.oauthSites.has(wiki.href) ) oauth.push(Wiki.oauthSites.get(wiki.href));
			}
...
globalThis.verifyOauthUser = function(state, access_token, settings) {
	...
	if ( !( ( Date.now() - settings.interaction?.createdTimestamp ) < 840_000 || channel.permissionsFor(channel.guild.members.me).has([PermissionFlagsBits.ViewChannel, ( channel.isThread() ? PermissionFlagsBits.SendMessagesInThreads : PermissionFlagsBits.SendMessages )]) ) ) return settings.fail?.();
	Promise.all([
